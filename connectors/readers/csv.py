...
from pyspark.sql.functions import expr, col, from_csv, lit
...
class CSVReader:  # pylint: disable=too-few-public-methods
    ...
    def read_csv(self, decrypt=False):
        ...
        if self.schema:
            ...
            df_valid = (
                df_csv.where(col(corrupt_record).isNull())
                .where(col(self.cache))
                .drop(corrupt_record)
                .drop(self.cache)
            )
            ...
            df_csv = self.__load_csv().drop(self.cache)
            if decrypt:
                df_csv = self.__decrypt_encrypted_cols(df_csv)
